...
router.post('/send-otp', async (req, res, next) => {
  try {
    ...
    // Save OTP with phone so resend works too
    await Otp.create({ email: email.toLowerCase(), phone: phone || null, otp: otpCode });
    ...
    if (phone) {
      await sendSms({ to: phone, template: 'otp', data: { otpCode } });
    }
...
router.post('/register', async (req, res, next) => {
  try {
    ...
    // Send welcome SMS if phone provided (non-blocking)
    if (phone) {
      sendSms({ to: phone, template: 'welcome', data: { name } }).catch(console.error);
    }
...
router.post('/forgot-password', async (req, res, next) => {
  try {
    ...
    // Send password reset SMS if user has a phone number
    if (user.phone) {
      sendSms({ to: user.phone, template: 'resetPassword', data: { resetUrl } }).catch(console.error);
    }
