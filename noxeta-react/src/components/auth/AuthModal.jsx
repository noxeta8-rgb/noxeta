...
export default function AuthModal({ onClose }) {
  ...
  const [form, setForm] = useState({ email: '', password: '', confirmPass: '', name: '', phone: '' })
  ...
  const doSignupSendOtp = async () => {
    ...
    if (!form.phone.trim()) { showToast('Error', 'Please enter your mobile number'); return }
    if (!/^[6-9]\d{9}$/.test(form.phone.replace(/\s/g, ''))) { showToast('Error', 'Enter a valid 10-digit Indian mobile number'); return }
    ...
    setLoading(true)
    const res = await sendOtp(form.email, form.phone)   // phone passed so OTP SMS fires
    setLoading(false)
    if (res.ok) {
      setStep('otp')
      setTimer(60)
      showToast('OTP Sent ✦', `Verification code sent to ${form.email} & ${form.phone}`)
    } else {
  ...
  const doVerifyAndSignup = async () => {
    ...
    const sRes = await signup(form.name, form.email, form.password, form.phone)  // phone saved to DB
    setLoading(false)
    if (sRes.ok) {
          ...
          {tab === 'signup' && step === 'form' && (
            <>
              ...
              <div className="form-group">
                ...
                <div style={{ display: 'flex', gap: '0' }}>
                  <span style={{
                    ...
                    color: 'var(--text-dim)', fontFamily: 'var(--font-b)',
                    fontSize: '15px', whiteSpace: 'nowrap',
                  }}>+91</span>
                  <input style={{ ...inp, borderLeft: 'none' }}
                    type="tel" placeholder="10-digit mobile number"
                    maxLength={10}
                    value={form.phone}
                    onChange={e => setForm(f => ({ ...f, phone: e.target.value.replace(/\D/g, '').slice(0, 10) }))} />
                </div>
                <p style={{ fontSize: '11px', color: 'var(--text-dim)', marginTop: '5px', fontFamily: 'var(--font-m)', letterSpacing: '0.5px' }}>
              ...
              <button className="btn-primary btn-block" onClick={doSignupSendOtp} disabled={loading}
                style={{ opacity: loading ? 0.7 : 1 }}>
                {loading ? 'Sending OTP...' : 'Verify Email & Mobile →'}
              </button>
            </>
          )}
          ...
          {step === 'otp' && (
            <>
              <p style={{ fontSize: '14px', color: 'var(--text-dim)', marginBottom: '28px', lineHeight: 1.7 }}>
                ...
                {form.phone && (
                  <> and SMS to <strong style={{ color: 'var(--text)' }}>+91 {form.phone}</strong></>
                )}.
